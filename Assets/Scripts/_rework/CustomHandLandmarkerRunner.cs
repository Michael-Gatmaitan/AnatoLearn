...
{
    /// <summary>
    /// Custom HandLandmarkerRunner that exposes events for hand landmark detection
    /// This allows other scripts to subscribe to hand landmark updates without using reflection
    /// </summary>
    public class CustomHandLandmarkerRunner : VisionTaskApiRunner<HandLandmarker>
    {
        [SerializeField]
        ...
        // Public events for hand landmark detection
        public event Action<HandLandmarkerResult> OnHandLandmarkDetected;
        public event Action<HandLandmarkerResult, Image, long> OnHandLandmarkDetectedAsync;
                ...
                switch (taskApi.runningMode)
                {
                    case Tasks.Vision.Core.RunningMode.IMAGE:
                        if (taskApi.TryDetect(image, imageProcessingOptions, ref result))
                        {
                            ...
                            // Fire event for synchronous detection
                    ...
                    case Tasks.Vision.Core.RunningMode.VIDEO:
                        ...
                        {
                            _handLandmarkerResultAnnotationController.DrawNow(result);
                            // Fire event for video detection
                            OnHandLandmarkDetected?.Invoke(result);
                        }
                        else
                        {
        ...
        )
        {
            ...
            // Fire event for async detection
            OnHandLandmarkDetectedAsync?.Invoke(result, image, timestamp);
        }
    }
}
